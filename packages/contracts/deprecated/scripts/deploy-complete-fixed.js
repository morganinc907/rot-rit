...
async function main() {
  ...
  // Deploy with explicit nonce management and higher gas price
  const deployOptions = {
    gasPrice: ethers.parseUnits("2", "gwei"), // Higher gas price for Base
  };
  ...
  const relics = await Relics.deploy("https://api.rotandrituals.com/relics/", { ...deployOptions, nonce: nonce++ });
  ...
  const cultists = await Cultists.deploy({ ...deployOptions, nonce: nonce++ });
  ...
  const demons = await Demons.deploy({ ...deployOptions, nonce: nonce++ });
  ...
  const cosmetics = await Cosmetics.deploy({ ...deployOptions, nonce: nonce++ });
  ...
  const keyShop = await KeyShop.deploy(await relics.getAddress(), { ...deployOptions, nonce: nonce++ });
  ...
  const txOptions = { gasPrice: ethers.parseUnits("2", "gwei") };

  // Relics authorizations
  console.log("🔗 Setting Relics authorizations...");
  await (await relics.setKeyShop(await keyShop.getAddress(), { ...txOptions, nonce: nonce++ })).wait();
  await (await relics.setMawSacrifice(await maw.getAddress(), { ...txOptions, nonce: nonce++ })).wait();
  await (await relics.setRitual(await rituals.getAddress(), { ...txOptions, nonce: nonce++ })).wait();
  console.log("✅ Relics authorizations set");

  // Raccoons
  console.log("🔗 Setting Raccoons ritual...");
  await (await raccoons.setRitual(await rituals.getAddress(), { ...txOptions, nonce: nonce++ })).wait();
  console.log("✅ Raccoons ritual set");

  // Cultists 
  console.log("🔗 Setting Cultists authorizations...");
  await (await cultists.setRitual(await rituals.getAddress(), { ...txOptions, nonce: nonce++ })).wait();
  await (await cultists.setMawSacrifice(await maw.getAddress(), { ...txOptions, nonce: nonce++ })).wait();
  console.log("✅ Cultists authorizations set");

  // Demons
  console.log("🔗 Setting Demons authorizations...");
  await (await demons.setRitual(await rituals.getAddress(), { ...txOptions, nonce: nonce++ })).wait();
  await (await demons.setMawSacrifice(await maw.getAddress(), { ...txOptions, nonce: nonce++ })).wait();
  console.log("✅ Demons authorizations set");

  // Cosmetics
  console.log("🔗 Setting Cosmetics authorizations...");
  await (await cosmetics.setMawSacrifice(await maw.getAddress(), { ...txOptions, nonce: nonce++ })).wait();
  await (await cosmetics.setCosmeticApplier(await applier.getAddress(), { ...txOptions, nonce: nonce++ })).wait();
  ...
  await (await maw.setMinBlocksBetweenSacrifices(1, { ...txOptions, nonce: nonce++ })).wait();
  await (await maw.setAshesPerVial(25, { ...txOptions, nonce: nonce++ })).wait();
  console.log("✅ MawSacrifice configured");

  // Demon tier URIs
  console.log("⚙️  Setting Demon tier URIs...");
  await (await demons.setTierBaseURI(1, "https://api.rotandrituals.com/demons/common/", { ...txOptions, nonce: nonce++ })).wait();
  await (await demons.setTierBaseURI(2, "https://api.rotandrituals.com/demons/rare/", { ...txOptions, nonce: nonce++ })).wait();
  await (await demons.setTierBaseURI(3, "https://api.rotandrituals.com/demons/legendary/", { ...txOptions, nonce: nonce++ })).wait();
  console.log("✅ Demon tier URIs set");

  // KeyShop price (0.002 ETH for testnet)
  console.log("⚙️  Setting KeyShop price...");
  await (await keyShop.setKeyPrice(ethers.parseEther("0.002"), { ...txOptions, nonce: nonce++ })).wait();
  ...
  await (await maw.setMonthlyCosmetics(1, [1, 2], { ...txOptions, nonce: nonce++ })).wait();
