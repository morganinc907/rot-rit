    ...
    UUPSUpgradeable
{
    ...
    function convertShardsToRustedCaps(uint256 shardAmount) external whenNotPaused whenConversionsNotPaused nonReentrant {
        ...
        // Removed broken canMintCaps check - staticcall always fails on state-changing functions
        // Let the actual mint/burn operations handle validation
    ...
    function canMintCaps(uint256 amount) external view returns (bool) {
        (bool success,) = address(relics).staticcall(
            abi.encodeWithSignature("mint(address,uint256,uint256,bytes)", address(this), RUSTED_CAP, amount, "")
        );
        return success;
    }
}