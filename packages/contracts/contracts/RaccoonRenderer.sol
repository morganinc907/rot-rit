...
contract RaccoonRenderer {
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        // Build traits array
        string memory traits = _buildTraits(rarity, headId, faceId, bodyId, furId, backgroundId);
        
        // Generate image URL
        string memory imageURL = _generateImageURL(tokenId, rarity, headId, faceId, bodyId, furId, backgroundId);
    ...
    function _generateImageURL(
        ...
        if (headId == 0 && bodyId == 0 && faceId == 0 && furId == 0 && backgroundId == 0) {
        ...
        return _generateSVGImage(tokenId, rarity, headId, bodyId, faceId, furId, backgroundId);
    }
    ...
    function _generateSVGImage(
        ...
        if (faceId > 0) {
            (, string memory faceImageURI,,,,,) = cosmetics.getCosmeticInfo(faceId);
            svg = string(abi.encodePacked(
                svg,
                '<image x="0" y="0" width="400" height="400" href="', faceImageURI, '"/>'
            ));
        }
        
        if (furId > 0) {
            (, string memory furImageURI,,,,,) = cosmetics.getCosmeticInfo(furId);
            svg = string(abi.encodePacked(
                svg,
                '<image x="0" y="0" width="400" height="400" href="', furImageURI, '"/>'
            ));
        }
        
        if (backgroundId > 0) {
            (, string memory backgroundImageURI,,,,,) = cosmetics.getCosmeticInfo(backgroundId);
            svg = string(abi.encodePacked(
                svg,
                '<image x="0" y="0" width="400" height="400" href="', backgroundImageURI, '"/>'
            ));
        }
    ...
    function _buildTraits(
        ...
        if (faceId > 0) {
            (string memory name,,, uint8 cosmeticRarity,,,) = cosmetics.getCosmeticInfo(faceId);
            traits = string(abi.encodePacked(
                traits,
        ...
        if (furId > 0) {
            (string memory name,,, uint8 cosmeticRarity,,,) = cosmetics.getCosmeticInfo(furId);
            traits = string(abi.encodePacked(
                traits,
                ', {"trait_type": "Fur", "value": "', name, '"}'
            ));
        }
        
        if (backgroundId > 0) {
            (string memory name,,, uint8 cosmeticRarity,,,) = cosmetics.getCosmeticInfo(backgroundId);
            traits = string(abi.encodePacked(
                traits,
        ...
        if (faceId > 0) cosmeticCount++;
        if (furId > 0) cosmeticCount++;
        if (backgroundId > 0) cosmeticCount++;
    ...
    function previewRaccoonWithCosmetic(
        ...
        return _generateImageURL(tokenId, rarity, headId, faceId, bodyId, furId, backgroundId);
    }
}
