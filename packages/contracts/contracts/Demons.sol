...
contract Demons is ERC721, Ownable, AccessControl {
    ...
    function setRitual(address ritual_) external onlyOwner {
        ...
        if (ritual != address(0)) {
            _revokeRole(MINTER_ROLE, ritual);
        }
        
        ritual = ritual_;
        if (ritual_ != address(0)) {
            _grantRole(MINTER_ROLE, ritual_);
        }
    ...
    function roles() external pure returns (string[] memory roleNames, bytes32[] memory roleHashes) {
        ...
        roleHashes = new bytes32[](2);
        
        roleNames[0] = "DEFAULT_ADMIN_ROLE";
        roleNames[1] = "MINTER_ROLE";
        
        roleHashes[0] = DEFAULT_ADMIN_ROLE;
        roleHashes[1] = MINTER_ROLE;
    }
