...
const LAYER_ORDER = [SLOT_INDICES.BACKGROUND, SLOT_INDICES.COLOR, SLOT_INDICES.BODY, SLOT_INDICES.FACE, SLOT_INDICES.HEAD];
...
export default function RaccoonDetail() {
  ...
  const [equippedLayers, setEquippedLayers] = useState({})
  
  const [selectedSlot, setSelectedSlot] = useState(null)
  const [activeTab, setActiveTab] = useState('wardrobe') // 'wardrobe' | 'wallet' | 'guide'
  
  const { raccoons, loading: raccoonsLoading, refetch: refetchRaccoons } = useRaccoons({
    baseUrl: '/abis',
    chainId
  })
  ...
  const handleEquip = async (slotIndex, itemIndex) => {
    ...
    // Refresh raccoon data to get updated tokenURI with cosmetics
    setTimeout(() => {
      refetchRaccoons()
    }, 3000) // Wait a bit longer than cosmetics refresh (2000ms)
  }
  
  const handleUnequip = async (slotIndex) => {
    await unequipSlot(slotIndex)
    
    // Refresh raccoon data to get updated tokenURI without cosmetics
    setTimeout(() => {
      refetchRaccoons()
    }, 3000)
  }
  ...
  // Fetch preview layers for equipped cosmetics (prefers transparent previewLayerURI)
  useEffect(() => {
    (async () => {
      try {
        if (!publicClient || !contractAddresses?.Cosmetics || !equipped) return;
        const addr = contractAddresses.Cosmetics;
        const next = {};
        for (const [name, slotIndex] of Object.entries(SLOT_INDICES)) {
          const idBig = equipped?.[slotIndex];
          if (idBig && idBig > 0n) {
            ...
            const imageURL = ipfsToHttp(data[1]);
            const previewURL = ipfsToHttp(data[2]);
            next[slotIndex] = previewURL || imageURL || '';
          }
        }
        setEquippedLayers(next);
      } catch (e) {
        console.warn('Failed to build equipped preview layers:', e);
      }
    })();
  }, [publicClient, contractAddresses, equipped]);
if (!isConnected) {
    return (
