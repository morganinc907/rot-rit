...
import { useKeyShopOptimistic } from '../hooks/useKeyShopOptimistic';
import { useContracts } from '../hooks/useContracts';
import OptimisticBalance from '../components/OptimisticBalance';
...
// MAW ABI for getting cosmetic pool (chain-first pattern)  
const MAW_COSMETICS_ABI = [
  {
    name: 'getCosmeticPool',
    type: 'function',
    stateMutability: 'view',
    inputs: [],
    outputs: [
...
export default function Store() {
  ...
  const { contracts: contractAddresses, isSupported, currentChainName, allowedChains, primaryChain } = useContracts();
  const { 
    keyPrice, 
    capBalance, 
    realCapBalance, 
  ...
  const handleBuyKeys = async () => {
    console.log("🛒 Buy Caps button clicked!", { keyAmount, totalCost, isPurchasing });
    ...
    try {
      console.log("🛒 Calling purchaseKeys function...");
      const result = await purchaseKeys(keyAmount);
      if (result.success) {
        setKeyAmount(1);
      }
    } catch (error) {
      console.error("Failed to buy caps:", error);
      // Don't show error toast here since useKeyShopOptimistic handles it
    }
  };
  ...
  // Fetch cosmetics from MAW contract (chain-first pattern)
  useEffect(() => {
    const fetchCosmetics = async () => {
      const mawAddress = contractAddresses?.MawSacrifice;
      const cosmeticsAddress = contractAddresses?.Cosmetics;
      
      console.log('🎨 Store useEffect - mawAddress:', mawAddress);
      console.log('🎨 Store useEffect - cosmeticsAddress:', cosmeticsAddress);
      console.log('🎨 Store useEffect - publicClient:', publicClient);
      
      // Block UI if required addresses are undefined or still loading
      if (!mawAddress || !cosmeticsAddress || !publicClient || !contractAddresses) {
      ...
      try {
        console.log('🎨 Fetching cosmetic pool from MAW (chain-first):', mawAddress);
        
        // Get current cosmetic pool from MAW contract (follows SYSTEM_ARCHITECTURE.md)
        const [availableIds, weights, totalWeight] = await publicClient.readContract({
          address: mawAddress,
          abi: MAW_COSMETICS_ABI,
          functionName: 'getCosmeticPool',
        });
        
        console.log('🎨 Available cosmetic IDs from MAW:', availableIds.map(id => Number(id)));
        console.log('🎨 Weights:', weights.map(w => Number(w)));
        console.log('🎨 Total weight:', Number(totalWeight));
        
        // Handle empty pool (no cosmetics configured yet)
        if (availableIds.length === 0 || Number(totalWeight) === 0) {
          console.log('🎨 No cosmetics configured in MAW pool');
          setCosmetics([]);
          setCurrentMonthlySet({ name: 'Fragment Sacrifice Not Configured', id: 0 });
          setCosmeticsLoading(false);
          return;
        }
        
        // Fetch cosmetic info for each available type from CosmeticsV2 contract
        const cosmeticsData = [];
        for (let i = 0; i < availableIds.length; i++) {
          const typeId = availableIds[i];
          const weight = weights[i];
          const dropRate = ((Number(weight) / Number(totalWeight)) * 100).toFixed(1);
          ...
          try {
            ...
            // Convert IPFS URLs to HTTPS            // Convert IPFS URLs to HTTPS and pick best display URI
            let imageURL = typeData[1];
            let previewURL = typeData[2];
            const ipfsToHttp = (u) => (u && u.startsWith('ipfs://') ? u.replace('ipfs://', 'https://ipfs.io/ipfs/') : u);
            imageURL = ipfsToHttp(imageURL);
            previewURL = ipfsToHttp(previewURL);
            ...
            cosmeticsData.push({
              ...
              name: typeData[0],
              imageURI: displayURL,
              previewLayerURI: previewURL,
              rarity: typeData[3],
              slot: typeData[4],
              monthlySetId: Number(typeData[5]),
              active: typeData[6],
              dropRate, // MAW-derived
        ...
        console.log('🎨 Fetched cosmetics from MAW pool:', cosmeticsData);
        setCosmetics(cosmeticsData);
        setCurrentMonthlySet({ name: 'Cap Ritual Catalog', id: 1 });
      } catch (error) {
        console.error('🎨 Error fetching cosmetics from MAW:', error);
        setCosmetics([]);
        setCurrentMonthlySet({ 
          name: error.message.includes('getCosmeticPool') 
            ? 'MAW Contract Error' 
            : 'Cosmetics Loading Error', 
          id: 0 
        });
      } finally {
        setCosmeticsLoading(false);
  ...
  if (!isSupported) {
    const allowedChainNames = allowedChains?.map(id => {
      switch(id) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-black to-red-950 text-gray-200">
        ...
        <div className="text-center max-w-md">
          <p className="mb-4 text-xl">⚠️ Unsupported Network</p>
          <p className="mb-2">Currently on: <span className="text-yellow-400 font-bold">{currentChainName}</span></p>
          <p className="mb-4">Please switch to: <span className="text-green-400 font-bold">{allowedChainNames}</span></p>
          <p className="text-sm text-gray-400">
            This environment supports {allowedChainNames}. Switch networks in your wallet to continue.
          </p>
        </div>
      </div>
    );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <div className="space-y-4">
              <div className="bg-gray-800/50 p-4 rounded-lg">
                ...
                <div className="text-gray-300 mb-2">
                  <OptimisticBalance
                    label="Your Balance"
                    realValue={realCapBalance}
                    optimisticValue={capBalance}
                    pendingAmount={pendingPurchases}
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                disabled={isPurchasing || !keyAmount}
                style={{ 
                  display: 'block', 
                  visibility: 'visible',
                  width: '100%',
              ...
              >
                {isPurchasing ? 'Purchasing...' : `Buy ${keyAmount} Cap${keyAmount > 1 ? 's' : ''}`}
              </button>
            </div>
          </div>
        </div>

        {/* Fragment Sacrifice Cosmetics */}
          ...
          {cosmeticsLoading ? (
            <div className="text-center py-8">
              <p className="text-gray-400">
                Loading cosmetics from MAW pool...
              </p>
            </div>
          ) : !contractAddresses ? (
            <div className="text-center py-8">
              <p className="text-yellow-400">
                ⚠️ Contract addresses not available
              </p>
              <p className="text-gray-400 mt-2">
                Waiting for contract resolution...
              </p>
            </div>
          ) : cosmetics.length === 0 ? (
            <div className="text-center py-8">
                ...
                return (
                  ...
                  >
                    <div className="w-full aspect-square bg-gray-700 rounded-md mb-2 flex items-center justify-center overflow-hidden">
                      {cosmetic.imageURI ? (
                        <img src={cosmetic.imageURI} alt={cosmetic.name} className="w-full h-full object-cover" crossOrigin="anonymous" />
                      ) : (
            ...
            >
              <div className="text-center">
                <div className="w-32 h-32 bg-gray-700 rounded-lg mx-auto mb-4 flex items-center justify-center">
                  {showCosmeticDetails.imageURI ? (
                    <img 
                      ...
                      className="w-full h-full object-cover rounded-lg" crossOrigin="anonymous" 
                    />
                  ) : (
                ...
                <p className="text-gray-400 mb-2">
                  Fragment Sacrifice Drop Rate: <span className="text-green-400 font-bold">{showCosmeticDetails.dropRate}%</span>
                </p>
                <p className="text-gray-400 mb-4">
                  Available from the Maw by sacrificing Rusted Caps
                </p>
                <p className="text-sm text-gray-300 mb-6">
                  This cosmetic can be applied to any Raccoon you own. Once applied, it becomes permanently attached to that Raccoon. Obtain by sacrificing fragments at the Maw!
                </p>
                <button
                  onClick={() => setShowCosmeticDetails(null)}
