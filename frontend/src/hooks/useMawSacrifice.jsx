...
import { useAccount, useWriteContract, useWaitForTransactionReceipt, useChainId, useReadContract, useBlockNumber, usePublicClient, useWatchContractEvent } from 'wagmi';
...
export default function useMawSacrifice(onSacrificeComplete) {
  ...
  // Event-driven updates for sacrifice-related events
  useWatchContractEvent({
    address: MAW_SACRIFICE_ADDRESS,
    abi: MAW_SACRIFICE_ABI,
    eventName: 'CosmeticRitualAttempted',
    args: { user: address },
    onLogs(logs) {
      // A sacrifice event occurred for this user, refetch lastSacrificeBlock
      refetchLastBlock();
    },
    enabled: !!address && !!MAW_SACRIFICE_ADDRESS && isSupported,
  });

  // Listen for block events to update block number for cooldown calculations
  useWatchContractEvent({
    address: MAW_SACRIFICE_ADDRESS,
    abi: MAW_SACRIFICE_ABI,
    eventName: 'SacrificeAttempted', // Generic sacrifice event
    onLogs(logs) {
      // Any sacrifice happened, might affect cooldowns
      refetchLastBlock();
    },
    enabled: !!MAW_SACRIFICE_ADDRESS && isSupported,
  });
  ...
  useEffect(() => {
    if (isConfirmed && receipt && onSacrificeComplete && !resultShown && pendingTransactionType === 'sacrifice') {
            ...
            try {
              ...
              if (decoded.eventName === 'CosmeticRitualAttempted' && decoded.args.success) {
                ...
                cosmeticReceived = {
                  ...
                  name: `Cosmetic #${cosmeticTypeId}`, // Will be enhanced with actual data below
                  quantity: 1,
                  needsEnhancement: true // Flag to fetch real data
                };
              }
            } catch (e) {
      ...
      // Enhance cosmetic data with real contract info
      const enhancedRewards = await Promise.all(rewards.map(async (reward) => {
        if (reward.type === 'cosmetic' && reward.needsEnhancement && contracts?.Cosmetics) {
          try {
            // Fetch actual cosmetic info from contract
            const cosmeticInfo = await publicClient.readContract({
              ...
              args: [BigInt(reward.typeId)]
            });
            
            const [name, imageURI, previewURI, rarity, slot] = cosmeticInfo;
            
            return {
              ...reward,
              name: name || `Cosmetic #${reward.typeId}`,
              imageURI: imageURI,
            ...
            console.warn('Failed to enhance cosmetic data:', error);
            return reward; // Return original if enhancement fails
          }
        }
        return reward;
      }));
      
      // Show results with enhanced rewards
      onSacrificeComplete({
        type: enhancedRewards.length > 0 ? 'rewards' : 'nothing',
        rewards: enhancedRewards,
        burned: burned,
        message: enhancedRewards.length > 0 ? 
