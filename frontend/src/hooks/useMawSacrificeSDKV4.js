              ...
              try {
                ...
                if (log.topics && log.topics[0] === '0xc3d58168c5ae7397731d063d5bbf3d657854427343f4c083240f7aacaa2d0f62') {
                  ...
                  if (from === '0x0000000000000000000000000000000000000000' && to.toLowerCase() === address?.toLowerCase()) {
                    ...
                    // Map token IDs to reward names based on contract address
                    const relicNames = {
                      0: 'Rusted Cap',
                      2: 'Lantern Fragment',
                      4: 'Bone Dagger',
                    ...
                    const cosmeticNames = {
                      ...
                      4: 'Orange Cosmetic',
                      5: 'Underpants',
                      7: 'Common Ritual Charm',
                      8: 'Uncommon Shadow Pendant'
                    };
                    
                    // Check which contract this log is from
                    const COSMETICS_ADDRESS = '0x32640D260CeCD52581280e23B9DCc6F49D04Bdcb';
                    const isFromCosmetics = log.address.toLowerCase() === COSMETICS_ADDRESS.toLowerCase();
                    
                    let rewardName;
                    if (isFromCosmetics) {
                      rewardName = cosmeticNames[id] || `Cosmetic #${id}`;
                    } else {
                      rewardName = relicNames[id] || `Token #${id}`;
                    }
              ...
              receipt.logs.forEach(log => {
                try {
                  if (log.topics && log.topics[0] === '0xc3d58168c5ae7397731d063d5bbf3d657854427343f4c083240f7aacaa2d0f62') {
                    ...
                    if (from === '0x0000000000000000000000000000000000000000' && to.toLowerCase() === address?.toLowerCase()) {
                      // Same contract-aware mapping as above
                      const relicNames = {
                        0: 'Rusted Cap', 2: 'Lantern Fragment', 4: 'Bone Dagger', 
                        6: 'Glass Shards', 7: 'Soul Deed', 8: 'Binding Contract'
                      };
                      const cosmeticNames = {
                        1: 'Glasses', 2: 'Strainer', 3: 'Pink Cosmetic', 4: 'Orange Cosmetic',
                        5: 'Underpants', 7: 'Common Ritual Charm', 8: 'Uncommon Shadow Pendant'
                      };
                      
                      const COSMETICS_ADDRESS = '0x32640D260CeCD52581280e23B9DCc6F49D04Bdcb';
                      const isFromCosmetics = log.address.toLowerCase() === COSMETICS_ADDRESS.toLowerCase();
                      const rewardName = isFromCosmetics ? 
                        (cosmeticNames[id] || `Cosmetic #${id}`) : 
                        (relicNames[id] || `Token #${id}`);
